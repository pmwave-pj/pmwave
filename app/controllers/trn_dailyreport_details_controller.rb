class TrnDailyreportDetailsController < ApplicationController
    ...
    def edit
      ...
      if @report.create_ymd.present?
        @report.create_ymd_str = @report.create_ymd.strftime("%Y%m%d")
      end
    ...
    def create
      ...
      if @report.valid?
        ...
        #エラーがない場合
        #--------------
        if @report.create_ymd_str.present?
           @report.create_ymd = Date.new(
            @report.create_ymd_str[0..3].to_i,
            @report.create_ymd_str[4..5].to_i,
            @report.create_ymd_str[6..7].to_i)
        end
